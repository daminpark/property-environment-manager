...
const pill = (text, tone='') => `<span class="pill ${tone}">${escapeHtml(text)}</span>`;
const metric = (k, v, tone='') => `<div class="metric"><span class="key">${escapeHtml(k)}</span><span class="value ${tone}">${escapeHtml(v)}</span></div>`;
function renderVentilation(data) {
  ...
  return `<h2>Ventilation · ${escapeHtml(data.house_code)}</h2>${summary}<div class="grid">${data.zones.map(z => {
    const tone = !z.fan_available || z.sensor_stale || z.fan_state_mismatch ? 'bad' : z.should_run ? 'warn' : 'good';
    return `<article class="card"><h3>${escapeHtml(String(z.zone_id ?? '').toUpperCase())} <span class="${tone}">${escapeHtml(z.mode)}</span></h3>${[
    ...
    ].join('')}<div class="reason">${escapeHtml(z.reason)}</div></article>`;
...
function renderTrv(data) {
  ...
  const boilerCard = `<article class="card"><h3>Boiler <span class="${boilerTone}">${escapeHtml(boiler.mode || 'starting')}</span></h3>${[
  ...
  ].join('')}<div class="reason">${escapeHtml(boiler.reason || 'waiting for first poll')}</div></article>`;
  const healthTone = data.active_control_ready_now ? 'good' : 'bad';
  ...
  return `<h2>TRV · ${escapeHtml(data.house_code)}</h2>${summary}<div class="grid">${boilerCard}${data.zones.map(z => {
...
function renderSummaries(items, kind) {
  ...
  if (!summaries.length) return `<div class="events"><div class="empty">No ${escapeHtml(kind)} daily summaries recorded yet.</div></div>`;
  return `<div class="events">${summaries.map(s => {
    ...
    return `<div class="event"><span>${escapeHtml(s.day)}</span><span>${escapeHtml(String(s.zone_id ?? '').toUpperCase())}</span><span class="${tone}">daily</span><span class="reason">better ${escapeHtml(m.would_improve_current_system || 0)} · worse ${escapeHtml(worse)} · blockers ${escapeHtml(m.hard_safety_blockers || 0)}</span></div>`;
  }).join('')}</div>`;
}
async function load() {
  ...
  const calendarState = !controllers.trv ? null : !controllers.trv.calendar_policy_enabled ? 'off' : controllers.trv.active_calendar_policy ? 'active' : 'observed';
  ...
  document.getElementById('status').innerHTML = [
    ...
    calendarState ? pill(`calendar ${calendarState}`, calendarState === 'active' ? 'warn' : calendarState === 'observed' ? 'good' : 'warn') : ''
  ].join('');
